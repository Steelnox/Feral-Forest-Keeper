...
public class Movement_PlayerState : State
{
    ...
    {
        if (SceneManager.GetActiveScene().name == "Lvl1 Def") CameraController.instance.SetFieldOfViewSmoothness(2.0f, CameraController.instance.standardFOV);
        ...
        if (SceneManager.GetActiveScene().name == "Lvl1 Def") CameraController.instance.p_Camera.transform.rotation = Quaternion.Lerp(CameraController.instance.p_Camera.transform.rotation, CameraController.instance.initCameraRotation, CameraController.instance.smoothValue * Time.deltaTime);
